...
| bit 11～12           | 그림자 종류             | 1             | 실선(두꺼운 선) 점선) 일점쇄선 (-.-.-.-.) 이점쇄선 (-..-..-..) 없음 비연속 |
| bit 13 14           | 양각 여부 음각 여부        | 2             | 연속                                                    |
| bit bit 15 bit      | 위 첨자 여부            |               |                                                       |
| 16 bit 17           | 아래 첨자 여부 Reserved. |               |                                                       |
| bit 18～20 bit 21～24 | 취소선 여부 강조점 종류      | 0 1 2 3 4     | 없음 검정 동그라미 강조점 속 빈 동그라미 강조점 ˇ ˜                       |
|                     | 어울리는 빈칸 사용 여부      |               |                                                       |
|                     | 취소선 모양             |               | :                                                     |
| bit                 |                    |               |                                                       |
| 25                  |                    |               |                                                       |
...
Tag  ID  :  HWPTAG\_TAB\_DEF

| 자료형             | 길이(바이트)         | 설명               |
|-----------------|-----------------|------------------|
| UINT32          | 4               | 속성               |
| INT16           | 4               | count            |
| 탭 정보 (count 개수) | 탭 정보 (count 개수) | 탭 정보 (count 개수)  |
| HPWUNIT         | 4               | 탭의 위치            |
| UINT8           | 1               | 탭의 종류            |
| UINT8           | 1               | 값                |
| UINT8           | 1               | 0 왼쪽             |
...
Tag  ID  :  HWPTAG\_NUMBERING

| 자료형              | 길이(바이트)   | 설명                                                                                                                                                                                          | 설명                                        |
|------------------|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
| BYTE stream      | 8         | 문단 머리 정보(표 39 참조)                                                                                                                                                                           |                                           |
| WORD             | 2         | 번호 형식 길이 (len)                                                                                                                                                                              | 7회 반복 수준(1～7) 각 레벨에 해당하는 숫자 또는 문자         |
| WCHAR array[len] | 2×len     | 번호 형식. 불릿 문단의 경우에는 사용되지 않는다. 문자열 내 특정 문자에 제어코드(^)를 붙임으로써 글에서 표시되는 번호 문단 머리의 형식을 제어한다. ^n : 레벨 경로를 표시한다.(예: 1.1.1.1.1.1.1) ^N : 레벨 경로를 표시하며 마지막에 마침표를 하나 더 찍는다.(예: 1.1.1.1.1.1.1.) 표 36 참조) | 또는 기호를 표시                                 |
| UINT16           | 2         | 시작 번호                                                                                                                                                                                       | 시작 번호                                     |
| UINT             | 4         | 수준별 시작번호 (5.0.2.5 이상)                                                                                                                                                                       | 7회 반복 수준(1～7)                             |
| WORD             | 2         | 확장 번호 형식 길이 (len)                                                                                                                                                                           | 3회 반복 수준(8～10) 각 레벨에 해당하는 숫자 또는 문자 기호를 표시 |
| WCHAR array[len] | 2×len     | 확장 번호 형식. 불릿 문단의 경우에는 사용되지 않는다. 문자열 내 특정 문자에 제어코드(^)를 붙임으로써 글에서 표시되는 번호 문단 머리의 형식을 제어한다. ^n : 레벨 경로를 표시한다.(예:                                                                              | 또는                                        |

|       |    | 1.1.1.1.1.1.1) ^N : 레벨 경로를 표시하며 마지막에 마침표를 하나 더 찍는다.(예: 1.1.1.1.1.1.1.) 표 36 참조)   |                                                    |
|-------|----|-----------------------------------------------------------------------------------|----------------------------------------------------|
| UINT  | 4  | 확장 수준별 시작번호 (5.1.0.0 이상)                                                          | 수준(8~10)                                           |
| 전체 길이 | 가변 | 10 + (2×len))×7 + 2 + (4×7) (2×len))×3 + (4×3) 바이트                                | 10 + (2×len))×7 + 2 + (4×7) (2×len))×3 + (4×3) 바이트 |
...
| bit 0～1 | 문단의 정렬 종류                      |   2 | 오른쪽             |
| bit 2   | 번호 너비를 실제 인스턴스 문자열의 너비에 따를지 여부 |   0 | false           |
| bit 2   | 번호 너비를 실제 인스턴스 문자열의 너비에 따를지 여부 |   1 | true            |
| bit 3   | 자동 내어 쓰기 여부                    |   0 | false           |
| bit 3   | 자동 내어 쓰기 여부                    |   1 | true            |
| bit 4   | 수준별 본문과의 거리 종류                 |   0 | 글자 크기에 대한 상대 비율 |
| bit 4   | 수준별 본문과의 거리 종류                 |   1 | 값               |
...
| bit 0～1 | 줄 간격 종류. 글 2007 이하 버전에서 사용. | 1   | 고정값       |
| bit 0～1 | 줄 간격 종류. 글 2007 이하 버전에서 사용. | 2   | 여백만 지정    |
| bit 2～4 | 정렬 방식                        | 0   | 양쪽 정렬     |
| bit 2～4 | 정렬 방식                        | 1   | 왼쪽 정렬     |
| bit 2～4 | 정렬 방식                        | 2   | 오른쪽 정렬    |
| bit 2～4 | 정렬 방식                        | 3   | 가운데 정렬    |
| bit 2～4 | 정렬 방식                        | 4   | 배분 정렬     |
| bit 2～4 | 정렬 방식                        | 5   | 나눔 정렬     |
| bit 5～6 | 줄 나눔 기준 영어 단위                | 0   | 단어        |
| bit 5～6 | 줄 나눔 기준 영어 단위                | 1   | 하이픈       |
| bit 5～6 | 줄 나눔 기준 영어 단위                | 2   | 글자        |
...
| bit 0~4 | 줄 간격 종류 |   1 | 고정 값   |
| bit 0~4 | 줄 간격 종류 |   2 | 여백만 지정 |
| bit 0~4 | 줄 간격 종류 |   3 | 최소     |

## 4.2.11. 스타일(문단 스타일)

Tag  ID  :  HWPTAG\_STYLE
...
| 범위      | 구분     | 값   | 설명            |
|---------|--------|-----|---------------|
| bit 0～2 | 스타일 종류 | 0 1 | 문단 스타일 글자 스타일 |
...
Tag  ID  :  HWPTAG\_DISTRIBUTE\_DOC\_DATA
...
Tag  ID  :  HWPTAG\_COMPATIBLE\_DOCUMENT
...
Tag  ID  :  HWPTAG\_LAYOUT\_COMPATIBILITY
...
## 4.3.1. 문단 헤더 Tag ID : HWPTAG\_PARA\_HEADER
...
텍스트의 수가 1 이상이면 문자 수만큼 텍스트를 로드하고 그렇지 않을 경우 PARA\_BREAK로 문단을 생성한다.

## 4.3.2. 문단의 텍스트

Tag  ID  :  HWPTAG\_PARA\_TEXT
...
문단은 최소 하나의 문자 Shape buffer가 존재하며, 첫 번째 pos가 반드시 0이어야 한다. 예를 들어 문자길이 40자
짜리 문단이 10자씩 4가지 다른 글자 모양으로 구성되어 있다면 buffer는 다음과 같다.
...
Tag  ID  :  HWPTAG\_PARA\_CHAR\_SHAPE
...
Tag  ID  :    HWPTAG\_PARA\_LINE\_SEG
...
| INT32  |         4 | 컬럼에서의 시작 위치                                                                         |
| INT32  |         4 | 세그먼트의 폭                                                                             |
| UINT32 |         4 | 태그 -bit 0 : 페이지의 첫 줄인지 여부 -bit 1 : 컬럼의 첫 줄인지 여부 -bit 16 : 텍스트가 배열되지 않은 빈 세그먼 트인지 여부 |
| UINT32 |         4 | -bit 17 : 줄의 첫 세그먼트인지 여부                                                            |
| UINT32 |         4 | -bit 18 : 줄의 마지막 세그먼트인지 여부                                                          |
| UINT32 |         4 | -bit 19 : 줄의 마지막에 auto-hyphenation이                                                 |
| UINT32 |         4 | 수행되었는지 여부.                                                                          |
| UINT32 |         4 | -bit 20 : indentation 적용                                                            |
| UINT32 |         4 | -bit 21 : 문단 머리 모양 적용                                                               |
| UINT32 |         4 | -bit 31 : 구현상의 편의를 위한 property                                                      |
| 전체 길이  |        36 |                                                                                     |
...
range tag 정보를 정보 수만큼 읽어 온다. range tag는 텍스트의 일정 영역을 마킹하는 용도로 사용되 며,  글자
모양과는 달리 각 영역은 서로 겹칠 수 있다.(형광펜, 교정 부호 등)

Tag  ID  :  HWPTAG\_PARA\_RANGE\_TAG
...
Tag  ID  :  HWPTAG\_CTRL\_HEADER

| 자료형    | 길이(바이트)   | 설명                                           |
|--------|-----------|----------------------------------------------|
| UINT32 | 4         | 컨트롤 ID                                       |
|        |           | 컨트롤 ID 이하 속성들은 CtrlID에 따라 다르다. - 컨트롤 및 개체 참고 |
| 전체 길이  | 4         |                                              |
...
Tag  ID  :  HWPTAG\_LIST\_HEADER

| 자료형    |   길이(바이트) | 설명      | 설명      | 설명   | 설명               |
|--------|-----------|---------|---------|------|------------------|
| INT16  |         2 | 문단 수    | 문단 수    | 문단 수 | 문단 수             |
| UINT32 |         4 | 속성      | 속성      | 속성   | 속성               |
| UINT32 |         4 | 범위      | 구분      | 값    | 설명               |
| UINT32 |         4 | bit 0～2 | 텍스트 방향  | 0    | 가로               |
| UINT32 |         4 | bit 0～2 | 텍스트 방향  | 1    | 세로               |
| UINT32 |         4 | bit 3～4 | 문단의 줄바꿈 | 0    | 일반적인 줄바꿈         |
| UINT32 |         4 | bit 3～4 | 문단의 줄바꿈 | 1    | 자간을 조종하여 한 줄을 유지 |
| UINT32 |         4 | bit 3～4 | 문단의 줄바꿈 | 2    | 내용에 따라 폭이 늘어남    |
| UINT32 |         4 | bit 5～6 | 세로 정렬   | 0    | top              |
| UINT32 |         4 | bit 5～6 | 세로 정렬   | 1    | center           |
| UINT32 |         4 | bit 5～6 | 세로 정렬   | 2    | bottom           |
| 전체 길이  |         6 |         |         |      |                  |
...
extended type의 컨트롤은 종류를 나타내는 식별 기호로 32 비트 ID가 사용된다. 컨트롤 코드가 큰 범주를 나타내는
식별 기호라고 한다면, 컨트롤 ID는 세부 분류를 나타내는 식별 기호인 셈이다. 예를  들어  단  정의  컨트롤  ID는
MAKE\_4CHID('c',  'o',  'l',  'd')  같은  형식으로  정의한다. MAKE\_4CHID(a,  b,  c,
d)  (((a)  &lt;&lt;  24)  |  ((b)  &lt;&lt;  16)  |  ((c)  &lt;&lt;  8)  |  (d))

|    |                              | 컨트롤 ID                                                                                                                                                                                    | 개체 공통 속성   | 개체 요소 속성   |
|----|------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------|------------|
|  1 | 표                            | MAKE_4CHID('t', 'b', 'l', ' ')                                                                                                                                                            | √          |            |
|  2 | ( 그리기 개체 ) 선 사각형 타원 호 다각형 곡선 | MAKE_4CHID('$', 'l', 'i', 'n') MAKE_4CHID('$', 'r', 'e', 'c') MAKE_4CHID('$', 'e', 'l', 'l') MAKE_4CHID('$', 'a', 'r', 'c') MAKE_4CHID('$', 'p', 'o', 'l') MAKE_4CHID('$', 'c', 'u', 'r') | √          | √          |
...
| bit 3～4 | 세로 위치의 기준 (VertRelTo)    | 1   | page                                               |
| bit 3～4 | 세로 위치의 기준 (VertRelTo)    | 2   | para                                               |
| bit 5～7 | 세로 위치의 기준에 대한 상대적인 배열 방식 | 0   | VerRelTo이 'paper'나 'page' 이면 top, 그렇지 않으면 left     |
| bit 5～7 | 세로 위치의 기준에 대한 상대적인 배열 방식 | 1   | VerRelTo이 'paper'나 'page' 이면 center                |
| bit 5～7 | 세로 위치의 기준에 대한 상대적인 배열 방식 | 2   | VerRelTo이 'paper'나 'page' 이면 bottom, 그렇지 않으면 right |
| bit 5～7 | 세로 위치의 기준에 대한 상대적인 배열 방식 | 3   | VerRelTo이 'paper'나 'page' 이면 inside                |
| bit 5～7 | 세로 위치의 기준에 대한 상대적인 배열 방식 | 4   | VerRelTo이 'paper'나 'page' 이면 outside               |

|           |                                           | 0                              | page                                    |
|-----------|-------------------------------------------|--------------------------------|-----------------------------------------|
| bit 8～9   | 가로 위치의 기준                                 | 1                              | page                                    |
|           | (HorzRelTo)                               | 2                              | column                                  |
|           |                                           | 3                              | para                                    |
| bit 10～12 | HorzRelTo에 대한 상대적인 배열 방식                  |                                | bit 5～7 참조                              |
| bit 13    | VertRelTo이 'para'일 때 오브젝트의 세로 위치를 본문 영역으로 | 0                              | off                                     |
| bit 13    | 제한할지 여부                                   | 1                              | on                                      |
| bit 14    | 다른 오브젝트와 겹치는 것을 허용할지 여부                   |                                | 오브젝트의 위치가 본문 영역으로 제한되면 언제나 false로 간주한다. |
| bit 15～17 | 오브젝트 폭의 기준                                | 0                              | paper                                   |
| bit 15～17 | 오브젝트 폭의 기준                                | 1                              | page                                    |
| bit 15～17 | 오브젝트 폭의 기준                                | 2                              | coloum                                  |
| bit 15～17 | 오브젝트 폭의 기준                                | 3                              | para                                    |
| bit 15～17 | 오브젝트 폭의 기준                                | 4                              | absolute                                |
| bit 18～19 | 오브젝트 높이의 기준                               | 0                              | paper                                   |
| bit 18～19 | 오브젝트 높이의 기준                               | 1                              | page                                    |
| bit 18～19 | 오브젝트 높이의 기준                               | 2                              | absolute                                |
| bit 20    | VertRelTo이 para일 때 크기 보호 여부               | 0                              | off                                     |
| bit 20    | VertRelTo이 para일 때 크기 보호 여부               | 1                              | on                                      |
| bit 21～23 | 오브젝트 주위를 텍스트가 어떻게 흘러갈지 지정하는 옵션            | 오브젝트 주위를 텍스트가 어떻게 흘러갈지 지정하는 옵션 | 오브젝트 주위를 텍스트가 어떻게 흘러갈지 지정하는 옵션          |
| bit 21～23 | Square                                    | 0                              | bound rect를 따라                          |
| bit 21～23 | Tight                                     | 1                              | 오브젝트의 outline을 따라                       |
| bit 21～23 | Through                                   | 2                              | 오브젝트 내부의 빈 공간까지                         |
| bit 21～23 | TopAndBottom                              | 3                              | 좌, 우에는 텍스트를 배치하지 않음                     |
| bit 21～23 | BehindText                                | 4                              | 글과 겹치게 하여 글 뒤로                          |
| bit 21～23 | InFrontOfText                             | 5                              | 글과 겹치게 하여 글 앞으로                         |
| bit 24～25 | 오브젝트의 좌/우 어느 쪽에 글을 배치할지 지정하는 옵션           | 0                              | BothSides                               |
| bit 24～25 | 오브젝트의 좌/우 어느 쪽에 글을 배치할지 지정하는 옵션           | 1                              | LeftOnly                                |
| bit 24～25 | 오브젝트의 좌/우 어느 쪽에 글을 배치할지 지정하는 옵션           | 2                              | RightOnly                               |
| bit 24～25 | 오브젝트의 좌/우 어느 쪽에 글을 배치할지 지정하는 옵션           | 3                              | LargestOnly                             |
| bit 26～28 | 이 개체가 속하는 번호 범주                           | 0                              | none                                    |
| bit 26～28 | 이 개체가 속하는 번호 범주                           | 1                              | figure                                  |
| bit 26～28 | 이 개체가 속하는 번호 범주                           | 2                              | table                                   |
| bit 26～28 | 이 개체가 속하는 번호 범주                           | 3                              | equation                                |
...
모든 그리기 개체에 대한 serialization은 우선 base인 그리기 개체 공통 속성을 serialize한 다음 자신이
가지고 있는 개체 요소 속성을 serialize한다.
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT(GenShapeObject일 경우 id가 두 번 기록됨)
...
|        |                         | 4 설명                      |
|        |                         | HWPUNIT16 INT32           |
| 2 4    | 회전각 회전 중심의 x 좌표(개체 좌표계) | INT32                     |
| 4      | 회전 중심의 y 좌표(개체 좌표계)     |                           |
|        | 참조)                     | 정보(표                      |
| n      | Rendering 79 바이트        | 전체 길이 가변 42+n             |

표 84 Rendering  정보

| 자료형         | 길이(바이트)   | 설명                                                                                           |
|-------------|-----------|----------------------------------------------------------------------------------------------|
| WORD        | 2         | scale matrix와 rotration matrix쌍의 개수(cnt) 초기엔 1, group할 때마다 하나씩 증가하고, ungroup할 때 마다 하나씩 감소한다. |
...
각 matrix는 원소가 double로 표현되는 3 X 3 matrix로 구현된다. 마지막 줄(row)은 항상 0, 0, 1이기
때문에 실제 serialization에서는 마지막 줄은 빠진다. 저장되는 정보는 다음과 같다.
...
| bit 6～9   | 선 끝 모양         | 1   | 개체들일 때는 "Flat"이 디폴트 flat  |
| bit 10～15 | 화살표 시작 모양      |     |                           |
| bit 22～25 | 화살표 끝 모양       | 0   | 모양 없음                     |
| bit 22～25 | 화살표 끝 모양       | 1   | 화살 모양                     |
| bit 22～25 | 화살표 끝 모양       | 2   | 오목한 화살모양                  |
| bit 22～25 | 화살표 끝 모양       | 3   | 속이 빈 다이아몬드 모양             |
| bit 22～25 | 화살표 끝 모양       | 4   | 속이 빈 원 모양                 |
| bit 22～25 | 화살표 끝 모양       | 5   | 속이 빈 사각 모양                |
| bit 22～25 | 화살표 끝 모양       | 6   | 속이 채워진 다이아몬드 모양           |
| bit 22～25 | 화살표 끝 모양       | 7   | 속이 채워진 원 모양               |
| bit 22～25 | 화살표 끝 모양       | 8   | 속이 채워진 사각 모양              |
|           | 화살표 시작 크기      |     |                           |
|           | 화살표 시작 크기      | 0   | 작은-작은                     |
|           | 화살표 시작 크기      | 1   | 작은-중간                     |
|           | 화살표 시작 크기      | 2   | 작은-큰                      |
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_LINE
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_RECTANGLE
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_ELLIPSE
...
| INT32  |         4 | end pos x                                                        |
| INT32  |         4 | end pos y                                                        |
| INT32  |         4 | start pos x2 interval of curve(effective only when it is an arc) |
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_POLYGON
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_ARC
...
Tag  ID  :  HWPTAG\_SHAPE\_COMPONENT\_CURVE
...
## 4.3.9.4. 그림 개체(HWPTAG\_SHAPE\_COMPONENT\_PICTURE)
...
| BYTE stream    | 8   | 안쪽 여백 정보(표 72 참조) 표(기본값 : 141), 그림(기본값 : 0) |
...
## 4.3.9.5. OLE 개체(HWPTAG\_SHAPE\_COMPONENT\_OLE)
...
| bit 16～21 | 개체 종류 | 1   | Embedded                                                                               |
| bit 16～21 | 개체 종류 | 2   | Link                                                                                   |
| bit 16～21 | 개체 종류 | 3   | Static                                                                                 |
| bit 16～21 | 개체 종류 | 4   | Equation                                                                               |
...
bit  0-7에  대한  자세한  설명은 MSDN을 참고 MFC COleClientItem::m\_nDrawAspect bit
8에  대한  자세한  설명은 MSDN을 참고

MFC COleClientItem::m\_bMoniker

## 4.3.9.6. 차트 개체

차트는 본문에 Ole 개체로 저장되며, 차트 내용은 Ole Compound 파일로 저장된다. 차트 파일의 최상위 storage에
"Contents", "OOXMLChartContents" stream이 저장될 수 있다.

기존 hwp(한글 Neo) 차트에서는 "Contents" stream만 존재한다.

한글  2018  한글에서는  OOXML  차트를  저장할  수  있고,  "OOXMLChartContents"와
"Contents" stream에 차트를 저장할 수 있다.

한글 2018에서 차트에서 "Contents" stream은 OOXML 차트를 구 버전(한글 Neo)의 차트로 변환한 예비
데이터이다.

"Contents" stream의 내용은 첨부 파일('한글문서파일형식\_차트\_revision1.2\_원본.hwp')의 내용을 참조하면
된다.

"OOXMLChartContents" stream은 MS OOXML에서 차트를 저장하는 방식으로, 최상위 element는
"chartSpace" element이며, 내용은 ISO/IEC 29500나 ECMA 376의 DrawingML의 Chart
부분을 참조하면 된다.

## 4.3.9.7. 묶음 개체(HWPTAG\_SHAPE\_COMPONENT\_CONTAINER)
...
## 4.3.9.8. 동영상 개체(HWPTAG\_VIDEO\_TDATA)
...
|----|-----------|-----------------------------------------------------------------|----------|----------------------------------------------------|
|  1 | 구역 정의     | MAKE_4CHID('s', 'e', 'c', 'd')                                  | √        | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  2 | 단 정의      | MAKE_4CHID('c', 'o', 'l', 'd')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  3 | 머리말 / 꼬리말 | MAKE_4CHID('h', 'e', 'a', 'd') / MAKE_4CHID('f', 'o', 'o', 't') | √        | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  4 | 각주 / 미주   | MAKE_4CHID('f', 'n', ' ', ' ') / MAKE_4CHID('e', 'n', ' ', ' ') | √        | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  5 | 자동번호      | MAKE_4CHID('a', 't', 'n', 'o')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  6 | 새 번호 지정   | MAKE_4CHID('n', 'w', 'n', 'o')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  7 | 감추기       | MAKE_4CHID('p', 'g', 'h', 'd')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  8 | 홀/짝수 조정   | MAKE_4CHID('p', 'g', 'c', 't')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
|  9 | 쪽 번호 위치   | MAKE_4CHID('p', 'g', 'n', 'p')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 10 | 찾아보기 표식   | MAKE_4CHID('i', 'd', 'x', 'm')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 11 | 책갈피       | MAKE_4CHID('b', 'o', 'k', 'm')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 12 | 글자 겹침     | MAKE_4CHID('t', 'c', 'p', 's')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 13 | 덧말        | MAKE_4CHID('t', 'd', 'u', 't')                                  |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 14 | 숨은 설명     | MAKE_4CHID('t', 'c', 'm', 't')                                  | √        | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
| 15 | 필드 시작     | 필드 컨트롤 ID                                                       |          | HWPTAG_CTRL_HEAD로 부터 시작하며 ctrlid로 각 개체를 확인 할 수 있다. |
...
| bit 1～2 | 제책 방법 |     | 위로 넘기기 |
| bit 1～2 | 제책 방법 | 2   |        |

## 4.3.10.1.2. 각주/미주 모양

Tag  ID  :  HWPTAG\_FOOTNOTE\_SHAPE
...
| 범위        | 구분                                     | 값                                  | 설명                                 |
|-----------|----------------------------------------|------------------------------------|------------------------------------|
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 0                                  | 1, 2, 3                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 1                                  | 동그라미 쳐진 1, 2, 3                    |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 2                                  | I, II, III                         |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 3                                  | i, ii, iii                         |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 4                                  | A, B, C                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 5                                  | a, b, c                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 6                                  | 동그라미 쳐진 A, B, C                    |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 7                                  | 동그라미 쳐진 a, b, c                    |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 8                                  | 가, 나, 다                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 9                                  | 동그라미 쳐진 가,나,다                      |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 10                                 | ㄱ, ㄴ, ㄷ                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 11                                 | 동그라미 쳐진 ㄱ,ㄴ,ㄷ                      |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 12                                 | 일, 이, 삼                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 13                                 | 一, 二, 三                            |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 14                                 | 동그라미 쳐진 一,二,三                      |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 15                                 | 갑, 을, 병, 정, 무, 기, 경, 신, 임, 계       |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 16                                 | 甲, 乙, 丙, 丁, 戊, 己, 庚, 辛, 壬, 癸       |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 0x80                               | 4가지 문자가 차례로 반복                     |
| bit 0～7   | 번호 모양. 0~16 은 범용. 0x80, 0x81은 각주/미주 전용 | 0x81                               | 사용자 지정 문자 반복                       |
| bit 8～9   | 페이지 내에서 각주를 다단에 위치시킬 방법                | 0                                  | (각주인 경우) 각 단마다 따로 배열               |
| bit 8～9   | 페이지 내에서 각주를 다단에 위치시킬 방법                | 1                                  | (각주인 경우) 통단으로 배열                   |
| bit 8～9   | 페이지 내에서 각주를 다단에 위치시킬 방법                | 2                                  | (각주인 경우) 가장 오른쪽 단에 배열              |
| bit 8～9   | 페이지 내에서 각주를 다단에 위치시킬 방법                | 0                                  | (미주인 경우) 문서의 마지막                   |
| bit 8～9   | 페이지 내에서 각주를 다단에 위치시킬 방법                | 1                                  | (미주인 경우) 구역의 마지막                   |
| bit 10～11 | numbering                              | 0                                  | 앞 구역에 이어서                          |
| bit 10～11 | numbering                              | 1                                  | 현재 구역부터 새로 시작                      |
| bit 10～11 | numbering                              | 2                                  | 쪽마다 새로 시작(각주 전용)                   |
| bit 12    | 각주 내용 중 번호 코드의 모양을 위 첨자 형식으로 할지 여부     | 각주 내용 중 번호 코드의 모양을 위 첨자 형식으로 할지 여부 | 각주 내용 중 번호 코드의 모양을 위 첨자 형식으로 할지 여부 |
| bit 13    | 텍스트에 이어 바로 출력할지 여부                     | 텍스트에 이어 바로 출력할지 여부                 | 텍스트에 이어 바로 출력할지 여부                 |

## 4.3.10.1.3. 쪽 테두리/배경

Tag  ID  :  HWPTAG\_PAGE\_BORDER\_FILL
...
| 범위      | 구분                  | 값   | 설명        |
|---------|---------------------|-----|-----------|
| bit 0～1 | 머리말이 적용될 범위(페이지 종류) | 0 1 | 양 쪽 짝수 쪽만 |
| bit 0～1 | 머리말이 적용될 범위(페이지 종류) | 2   | 홀수 쪽만     |
...
| bit 0～3  | 번호 종류       | 1   | 각주 번호                                           |
| bit 0～3  | 번호 종류       | 2   | 미주 번호                                           |
| bit 0～3  | 번호 종류       | 3   | 그림 번호                                           |
| bit 0～3  | 번호 종류       | 4   | 표 번호                                            |
| bit 0～3  | 번호 종류       | 5   | 수식 번호                                           |
...
| UINT  |         2 | 속성   | 속성         | 속성      |
| UINT  |         2 | 구분   | 값          | 설명      |
| UINT  |         2 | 구분   | 0x00000001 | 머리말     |
| UINT  |         2 | 구분   | 0x00000002 | 꼬리말     |
| UINT  |         2 | 구분   | 0x00000004 | 바탕쪽     |
| UINT  |         2 | 구분   | 0x00000008 | 테두리     |
| UINT  |         2 | 구분   | 0x00000010 | 배경      |
| UINT  |         2 | 구분   | 0x00000020 | 쪽 번호 위치 |
| 전체 길이 |         2 |      |            |         |
...
| UINT32 |         4 | 속성      | 속성      | 속성   | 속성   |
| UINT32 |         4 | 범위      | 구분      | 값    | 설명   |
| UINT32 |         4 | bit 0～1 | 홀/짝수 구분 | 0    | 양 쪽  |
| UINT32 |         4 | bit 0～1 | 홀/짝수 구분 | 1    | 짝수 쪽 |
| UINT32 |         4 | bit 0～1 | 홀/짝수 구분 | 2    | 홀수 쪽 |
| 전체 길이  |         4 |         |         |      |      |
...
| bit 8～11 | 번호의 표시 위치 | 1   | 왼쪽 위     |
| bit 8～11 | 번호의 표시 위치 | 2   | 가운데 위    |
| bit 8～11 | 번호의 표시 위치 | 3   | 오른쪽 위    |
| bit 8～11 | 번호의 표시 위치 | 4   | 왼쪽 아래    |
| bit 8～11 | 번호의 표시 위치 | 5   | 가운데 아래   |
| bit 8～11 | 번호의 표시 위치 | 6   | 오른쪽 아래   |
| bit 8～11 | 번호의 표시 위치 | 7   | 바깥쪽 위    |
| bit 8～11 | 번호의 표시 위치 | 8   | 바깥쪽 아래   |
| bit 8～11 | 번호의 표시 위치 | 9   | 안쪽 위     |
| bit 8～11 | 번호의 표시 위치 | 10  | 안쪽 아래    |
...
HWPTAG\_CTRL\_DATA로 레코드된다. (Tag: HWPTAG\_CTRL\_DATA 참조)
...
| 자료형   |   길이(바이트) | 설명           | 설명           | 설명             |
|-------|-----------|--------------|--------------|----------------|
| WORD  |         2 | main Text 길이 | main Text 길이 | main Text 길이   |
| WCHAR |         2 | main Text    | main Text    | main Text      |
| WORD  |         2 | sub Text 길이  | sub Text 길이  | sub Text 길이    |
| WCHAR |         2 | sub Text     | sub Text     | sub Text       |
| UINT  |         4 | 구분           | 값            | 설명             |
| UINT  |         4 | 덧말의 위치       | 0            | 위              |
| UINT  |         4 | 덧말의 위치       | 1            | 아래             |
| UINT  |         4 | 덧말의 위치       | 2            | 가운데            |
| UINT  |         4 | Fsizeratio   | Fsizeratio   | Fsizeratio     |
| UINT  |         4 | Option       | Option       | Option         |
| UINT  |         4 | Style number | Style number | Style number   |
| UINT  |         4 | 구분           | 값            | 설명             |
| UINT  |         4 | 정렬 기준        | 0            | 양쪽 정렬          |
| UINT  |         4 | 정렬 기준        | 1            | 왼쪽 정렬          |
| UINT  |         4 | 정렬 기준        | 2            | 오른쪽 정렬         |
| UINT  |         4 | 정렬 기준        | 3            | 가운데 정렬         |
| UINT  |         4 | 정렬 기준        | 4            | 배분 정렬          |
| UINT  |         4 | 정렬 기준        | 5            | 나눔 정렬(공백에만 배분) |
| 전체 길이 |        18 |              |              |                |
...
| 범위        | 구분                            | 값   | 설명         |
|-----------|-------------------------------|-----|------------|
| bit 0     | 읽기 전용 상태에서도 수정 가능한지           |     |            |
| bit 11～14 | 하이퍼링크 필드 업데이트 시 글자 속성 업데이트 종류 | 0x1 | 열어보지 않은 링크 |
| bit 11～14 | 하이퍼링크 필드 업데이트 시 글자 속성 업데이트 종류 | 0x2 | 열어본 링크     |
| bit 11～14 | 하이퍼링크 필드 업데이트 시 글자 속성 업데이트 종류 | 0x4 | 링크 생성      |
...
글 '문서 이력 관리 정보'는 글 2005(6.5.0.724), 문서 형식 버전(Doc 5.0.1.7)부터 지원한다. 글
메뉴의 '파일-문서 이력 관리'에서 표시 및 생성되는 문서의 이력 정보를 저장하는 장소이다.(글 2005에서 글 2007까지는
'버전 비교(파일-버전 비교)'라는 이름으로 '문서 이력 관리 정보' 기능을 제공하였다.)

문서 이력 정보의 각각의 아이템은 '히스토리' 혹은 '히스토리 아이템'이라 하며 글 Compound 구조 내에서 각 아이템은
'DocHistory'라는 스토리지 내부에 VersionLog%d(%d는 버전) 이름의 스트 림으로 저장된다. 또한, 각각
아이템은 압축, 암호화되어 저장된다. 이력 정보 데이터를 'DocHistory'라는 새로운 스토리지로 저장한다.

## 4.4.2. 문서 이력 관리 레코드 정보

## 4.4.2.1. 히스토리 아이템 정보 시작

Tag  ID  :  HISTORY\_RECORD\_TYPE\_STAG  (0x10)

| 내 용            | 첨부 데이터 길이 및   | Type        |
|----------------|---------------|-------------|
| 히스토리 아이템 정보 시작 | WORD UINT     | flag option |

## 표 154  히스토리 아이템 정보 시작

- flag  :  각  아이템에  대한  포함  레코드 flag

- HISTORY\_INFO\_FLAG\_VERSION

(0x01)  :  버전  존재

- HISTORY\_INFO\_FLAG\_DATE

(0x02)  :  날짜  존재

- HISTORY\_INFO\_FLAG\_WRITER

(0x04)  :  작성자  존재

- HISTORY\_INFO\_FLAG\_DESCRIPTION

(0x08)  :  설명  존재

- HISTORY\_INFO\_FLAG\_DIFFDATA

(0x10)  :  Diff  Data  존재

- HISTORY\_INFO\_FLAG\_LASTDOCDATA

:  최근  문서  존재  (기록하지 않음, 필수)

- HISTORY\_INFO\_FLAG\_LOCK

(0x40)  :  현재  히스토리 아이템 Lock 상태

✓ option :  버전  정보  관련  공통 옵션

·

HWPVERSION\_AUTOSAVE

(0x00000001)  :  문서  저장  시  자동  저장

## 4.4.2.2. 히스토리 아이템 정보 끝

Tag  ID  :  HISTORY\_RECORD\_TYPE\_ETAG  (0x11)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_VERSION  (0x20)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_DATE (0x21)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_WRITER (0x22)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_DESCRIPTION  (0x23)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_DIFFDATA (0x30)
...
Tag  ID  :  HISTORY\_RECORD\_TYPE\_LASTDOCDATA (0x31)
...
히스토리 아이템 저장 시 시작은 HISTORY\_RECORD\_TYPE\_STAG 레코드로 시작하며, 아이템 내용 의 끝은
HISTORY\_RECORD\_TYPE\_ETAG 레코드로 종료한다.
...
(우)  463-400 경기도 성남시 분당구 대왕판교로 644번길 49 한컴타워 10층 전화: (031)  627-7000 팩스:
(031)  627-7709